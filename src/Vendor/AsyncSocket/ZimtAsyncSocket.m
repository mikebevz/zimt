...
NSString *const ZimtAsyncSocketException = @"ZimtAsyncSocketException";
NSString *const ZimtAsyncSocketErrorDomain = @"ZimtAsyncSocketErrorDomain";

#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5
// Mutex lock used by all instances of ZimtAsyncSocket, to protect getaddrinfo.
// Prior to Mac OS X 10.5 this method was not thread-safe.
static NSString *getaddrinfoLock = @"lock";
#endif

enum ZimtAsyncSocketFlags
{
...
@interface ZimtAsyncSocket (Private)
...
/**
 * The ZimtAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface ZimtAsyncReadPacket : NSObject
{
  @public
...
@implementation ZimtAsyncReadPacket
...
/**
 * The ZimtAsyncWritePacket encompasses the instructions for any given write.
**/
@interface ZimtAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation ZimtAsyncWritePacket
...
/**
 * The ZimtAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface ZimtAsyncSpecialPacket : NSObject
{
  @public
...
@implementation ZimtAsyncSpecialPacket
...
@implementation ZimtAsyncSocket
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	// and that the read packet isn't an ZimtAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[ZimtAsyncReadPacket class]]) return NAN;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	// and that the write packet isn't an ZimtAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[ZimtAsyncWritePacket class]]) return NAN;
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	if (theDelegate == NULL)
    {
		[NSException raise:ZimtAsyncSocketException
...
- (void)doAcceptWithSocket:(CFSocketNativeHandle)newNative
{
	...
	// Note: We use [self class] to support subclassing ZimtAsyncSocket.
	ZimtAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog (@"ZimtAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"ZimtAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"ZimtAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		NSLog (@"ZimtAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}

	if(pass && !CFWriteStreamOpen (theWriteStream))
	{
		NSLog (@"ZimtAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the ZimtAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	NSError *err = nil;
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			NSLog (@"ZimtAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		NSLog(@"ZimtAsyncSocket couldn't determine IP version of socket");
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		// Check to see if it's a normal read packet (not ZimtAsyncSpecialPacket) and if it had read anything yet.

		if(([theCurrentRead isKindOfClass:[ZimtAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketCFSocketError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard ZimtAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketCanceledError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"Connection canceled", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketCanceledError userInfo:info];
}

/**
 * Returns a standard ZimtAsyncSocket connect timeout error.
**/
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketConnectTimeoutError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"Attempt to connect to host timed out", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketConnectTimeoutError userInfo:info];
}

/**
 * Returns a standard ZimtAsyncSocket maxed out error.
**/
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketReadMaxedOutError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard ZimtAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketReadTimeoutError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"Read operation timed out", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard ZimtAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"ZimtAsyncSocketWriteTimeoutError",
														 @"ZimtAsyncSocket", [NSBundle mainBundle],
														 @"Write operation timed out", nil);

	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

	return [NSError errorWithDomain:ZimtAsyncSocketErrorDomain code:ZimtAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"<ZimtAsyncSocket %p", self]];
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ZimtAsyncReadPacket *packet = [[ZimtAsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[ZimtAsyncSpecialPacket class]])
			{
...
- (void)doBytesAvailable
{
	...
	if((theCurrentRead != nil) && (theReadStream != NULL))
	{
		// Note: This method is not called if theCurrentRead is an ZimtAsyncSpecialPacket (startTLS packet)
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ZimtAsyncWritePacket *packet = [[ZimtAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[ZimtAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)doSendBytes
{
	if((theCurrentWrite != nil) && (theWriteStream != NULL))
	{
		// Note: This method is not called if theCurrentWrite is an ZimtAsyncSpecialPacket (startTLS packet)
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	ZimtAsyncSpecialPacket *packet = [[ZimtAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		ZimtAsyncSpecialPacket *tlsPacket = (ZimtAsyncSpecialPacket *)theCurrentRead;
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog (@"ZimtAsyncSocket %p received unexpected CFSocketCallBackType %lu.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"ZimtAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %lu.", self, type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"ZimtAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %lu.", self, type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	ZimtAsyncSocket *theSocket = [[(ZimtAsyncSocket *)pInfo retain] autorelease];
